...
var (
	vagrant = flag.Bool("vagrant", false, "Force to boot as a new node")
)
...
func IsVagrant(ipnet *net.IPNet, inVagrant bool) bool {
	if !inVagrant {
		return false
	}
...
func GetIntranetIP() []string {
	...
	for _, addr := range addrs {
		fmt.Println(addr)
		if ipnet, ok := addr.(*net.IPNet); ok && !ipnet.IP.IsLoopback() && ipnet.IP.To4() != nil && !IsDocker0(ipnet) && !IsVagrant(ipnet, *vagrant) && !IsMaskEmpty(ipnet) && !IsFlannel(ipnet) && !IsCalico(ipnet) && !IsTunl(ipnet) && !IsVirtual(ipnet) && !IsBridge(ipnet) {
